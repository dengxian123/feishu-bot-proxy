export default async function handler(req, res) {
  // CORS 允许 GitHub Pages、飞书妙搭和本地开发
  const origin = req.headers.origin || '';
  const allowed = ['https://dengxian123.github.io', 'https://fcnv9xqdh3po.feishu.cn', 'http://localhost'];
  ...
  if (!content || !chatId) {
    return res.status(400).json({ error: '缺少 content 或 chat_id', code: -1 });
  }
  ...
  try {
    ...
    // 第2步：发送消息到聊天框
    var msgRes = await fetch('https://open.feishu.cn/open-apis/im/v1/messages?receive_id_type=chat_id', {
      method: 'POST',
      headers: {
